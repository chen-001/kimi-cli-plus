...
class AskUserParams(BaseModel):
    ...
    require_input: bool = Field(
        default=True, description="是否需要用户输入。如果为false且提供了options，则只需用户确认即可"
    )
...
class AskUserResult(BaseModel):
    ...
    choice_index: int | None = Field(default=None, description="如果提供了选项，这是用户选择的索引")
...
class AskUser(CallableTool2[AskUserParams]):
    ...
    async def __call__(self, params: AskUserParams) -> ToolReturnValue:
        ...
        # 添加选项（如果有）
        if params.options:
            options_text = "\n".join(f"  {i + 1}. {opt}" for i, opt in enumerate(params.options))
        ...
        result = AskUserResult(response="[等待用户输入...]", cancelled=False)
