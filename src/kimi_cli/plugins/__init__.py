...
def apply_all_patches():
    ...
    print("[Plugin] Loading user plugins v{}...".format(PLUGIN_VERSION))
    
    # 导入并应用各个补丁模块
    # 注意：从模块导入，不是从函数导入
    try:
        from kimi_cli.plugins.patches import visualize_patch as viz_module
        if hasattr(viz_module, 'patch') and callable(viz_module.patch):
            if viz_module.patch():
                _applied_patches.append("visualize")
                print("[Plugin] ✓ visualize patch applied")
    except Exception as e:
        warnings.warn(f"[Plugin] visualize patch failed: {e}", RuntimeWarning)
    
    try:
        from kimi_cli.plugins.patches import kimisoul_patch as ks_module
        if hasattr(ks_module, 'patch') and callable(ks_module.patch):
            if ks_module.patch():
                _applied_patches.append("kimisoul")
                print("[Plugin] ✓ kimisoul patch applied")
    except Exception as e:
        warnings.warn(f"[Plugin] kimisoul patch failed: {e}", RuntimeWarning)
    
    try:
        from kimi_cli.plugins.patches import mod_tracker_patch as mt_module
        if hasattr(mt_module, 'patch') and callable(mt_module.patch):
            if mt_module.patch():
