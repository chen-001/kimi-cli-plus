...
def check_compatibility(
    ...
    if kimi_version is None:
        return False, "无法检测kimi-cli-plus版本"

    kimi_tuple = parse_version(kimi_version)
    min_tuple = parse_version(min_kimi_version)

    if kimi_tuple < min_tuple:
        return False, f"kimi-cli-plus版本过低。需要 >= {min_kimi_version}，当前 {kimi_version}"

    if max_kimi_version:
        max_tuple = parse_version(max_kimi_version)
        if kimi_tuple > max_tuple:
            return False, f"kimi-cli-plus版本过高。插件可能不兼容当前版本 {kimi_version}"

    return True, f"版本兼容 (kimi-cli-plus {kimi_version}, 插件 {plugin_version})"
